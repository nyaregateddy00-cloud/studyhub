...
import { generateRevisionPlan } from "@/lib/study-ai.functions";
...
export const Route = createFileRoute("/_app/planner")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Schedule sessions, track goals and auto-build revision plans." },
    ],
  }),
  component: Planner,
});
...
function Planner() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { tasks: tasks.data ?? [], goals: goals.data ?? [] };
    },
  });
  ...
  const toggleTask = useMutation({
    mutationFn: async (task: { id: string; completed: boolean; duration_minutes: number; subject: string | null }) => {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-muted-foreground">
          Schedule sessions, set goals and let AI lay out your revision.
        </p>
      </div>
